...
import Text.Regex.Deriv.Common (Range(..), PosEpsilon(..), myHash, GFlag(..), IsGreedy(..), preBinder, subBinder, mainBinder)
import Text.Regex.Deriv.IntPattern (Pat(..), strip, listifyBinder)
import Text.Regex.Deriv.Parse
import qualified Text.Regex.Deriv.Dictionary as D (nub)
...
combineRange ((r1@(Range b1 e1)):_) ((r2@(Range b2 e2)):_) 
  | b1 == b2 = if e1 >= e2 
               then [r1]
               else [r2]
  | b1 > b2 = [r1]
...
toSBinder (PVar x _ p) = SVar (x,[]) (toSBinder p) emptyCF
toSBinder (PE _) = SRE emptyCF
toSBinder (PStar _ _) = SStar emptyCF
toSBinder (PPair p1 p2) = SPair (toSBinder p1) (toSBinder p2) emptyCF
toSBinder (PChoice ps _) = SChoice (map toSBinder ps) emptyCF
...
dPat0 (PChoice [] _) _ = mzero
dPat0 y@(PChoice [!p] _) l = do
      { (!p',!f') <- dPat0C p l
...
nub2Choice [] _ = return (PChoice [] Greedy, (\_ !sb -> {-# SCC "nubChoice/id0" #-} sb )) -- the base case is the identity
nub2Choice ([]:pfs) pDict = do 
     { (PChoice !ps !g, !f'') <- nub2Choice pfs pDict
     ; let f' !i !sb = {-# SCC "nub2Choice/f1" #-} case sb of
           { SChoice (_:ss) !cf -> ss `seq`
                f'' i $! (SChoice ss cf)
    ...
    case p of 
      { PChoice !ps' _ -> do 
        { let fs' :: [Int -> SBinder -> SBinder]
              fs' = repeat (\_ !sb -> {-# SCC "nubChoice/id1" #-}  sb ) --  identity functions
              pfs' = zip ps' fs'
...
carryForward _ sb2 = error ("trying to carry forward into a non-annotated pattern binder " ++ (show sb2))
...
instance Ord Pat where
  ...
  compare (PChoice ps1 g1) (PChoice ps2 g2) = let r = {-# SCC "compare7" #-} compare g1 g2 in case r of
     { EQ -> compare ps1 ps2
     ; _  -> r }
  compare p1 p2 = {-# SCC "compare8" #-} compare (assignIntToPat p1) (assignIntToPat p2) 

assignIntToPat :: Pat -> Int
assignIntToPat (PVar _ _ _) = 0
assignIntToPat (PE _) = 1
assignIntToPat (PStar _ _) = 2
assignIntToPat (PPair _ _) = 3
assignIntToPat (PChoice _ _) = 4
-- extract a carry forward from the sbinder

extract :: Pat -> SBinder -> CarryForward
extract (PVar !x _ !p) (SVar (_,!b) !sb !cf)
     | posEpsilon (strip p) = let cf' = extract p sb
                                  cf'' = cf' `seq` insertCF (x,b) cf'
                              in  cf'' `seq` (cf'' `combineCF` cf)
     | otherwise = IM.empty -- cf?
extract (PE _) (SRE !cf) = cf
extract (PStar _ _) (SStar !cf) = cf
extract (PPair !p1 !p2) (SPair !sb1 !sb2 !cf) = let cf1 = (extract p1 sb1) 
                                                    cf2 = (extract p2 sb2) 
                                                    cf' = cf1 `seq` cf2 `seq` (combineCF cf1 cf2)
                                                 in  cf' `seq` (cf' `combineCF` cf)
extract (PChoice !ps _) (SChoice !sbs !cf) =  let psbs = zip ps sbs 
...
sbinderToEnv' (PVar _ _ p) (SVar sr sb cf) 
...
buildDfaTable p = 
  let sig = sigmaRE (strip p)
      ...
      (delta, _) = {-# SCC "buildDfaTable/builder" #-}  builder sig [] init_dict 0 [p] -- 0 is already used by p
      delta' = delta
      table = {-# SCC "buildDfaTable/table" #-} IM.fromList (map (\ (s,c,d,f,sb2env) -> (myHash s c, (d,f,sb2env))) delta')
...
testp =
   ...
   let (Right (pp,_)) = parsePatPosix "^[XY]*X([XY]?){1,2}Y[XY]*$"
   -- let (Right (pp,posixBnd)) = parsePatPosix "^.*X(.?){1,4}Y.*$"
   -- let (Right (pp,posixBnd)) = parsePatPosix "X(.?){1,4}Y"         
   in pp

testp2 :: (Pat, FollowBy, IM.IntMap ())
testp2 = 
   let (Right (pp,posixBnd)) = parsePatPosix "X(.?){1,3}Y"
       fb                    = followBy pp
   in (pp,fb,posixBnd)

testp3 :: ([(Int, Char, Int, Int -> SBinder -> SBinder,
             SBinder -> [Env])],
             M.Map Pat Int)
testp3 = 
...
builder sig acc_delta dict max_id curr_pats 
   ...
   | otherwise = 
      let 
          ...
          new_pats       = {-# SCC "builder/new_pats" #-} D.nub [ p' | (_,_,p',_,_) <- new_delta, not (p' `M.member` dict) ] -- todo D.nub might cause collision
          (dict',max_id') = {-# SCC "builder/dict'" #-} foldl' (\(d,idd) p -> (M.insert p (idd+1) d, idd + 1)) (dict,max_id) new_pats
     ...
     case {-# SCC "execDfa/uncons" #-} S.uncons w' of
       ...
       Just (l,w) -> 
          let ((i,sb,_):_) = {-# SCC "execDfa/currDfaStateSBinders" #-} currDfaStateSBinders
              k            = {-# SCC "execDfa/myHash" #-} myHash i l
...
execPatMatch (dt, init_sbinder, init_sb2env, _, _, posixBinder) w = 
  let r = {-# SCC "execDfa" #-} dt `seq` execDfa 0 dt w [(0, init_sbinder, init_sb2env)]
  in case r of 
   { [] -> Nothing 
   ; ((_,sb,sb2env):_) -> case (sb2env sb) of 
...
testp4 :: Pat
testp4 = PVar 0 [] (PPair (PVar 1 [] ((PPair p_x p_y))) p_z)
...
testp5 :: Pat
testp5 = PVar 0 [] (PStar ( PVar 1 [] ( PChoice [(PVar 2 [] (PChoice [p3,p4] Greedy)), p5] Greedy)) Greedy)
...
patToRegex :: Pat -> a -> b -> c
              -> (DfaTable, SBinder, SBinder -> [Env], [Int], FollowBy, a)
patToRegex p posixBnd _ _ =  
 ...
 case execPatMatch r bs of
   ...
   Just env ->
     let pre = case lookup preBinder env of { Just e -> rg_collect_many bs e ; Nothing -> S.empty }
         ...
         matched = map ((rg_collect_many bs) . snd) (filter (\(v,_) -> v > mainBinder && v < subBinder ) env)
...
rg_collect_many :: S.ByteString -> [Range] -> S.ByteString
...
instance RegexOptions Regex CompOption ExecOption where
    ...
    setExecOpts _ _ = undefined
    getExecOpts _ = undefined 
