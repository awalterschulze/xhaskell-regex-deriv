...
isPosixBinder g =  do { st <- get
                       ; return (IM.member g (posix_binder st)) 
                       }                           
...
translate epat = case runState (trans epat) initTState of
                 (pat, st) ->
                   let hasAnchorS = anchorStart st
                       hasAnchorE = anchorEnd st
...
adhocSimp q@(PChoice [ (PVar x _ p) ] _) = do 
  { b <- isPosixBinder x
  ; if b 
    then return p 
    else return q }
...
r_trans epat = 
    ...
    ; EOr es ->
      ...
      do { rs <- mapM r_trans es
         ; case rs of
           { [] -> return Phi
           ; (q:qs) -> return (Choice (q:qs) Greedy)
           }
         }
    ; EConcat es ->
      {-
      ...
      do { rs <- mapM r_trans es
         ; case rs of
           { [] -> return Empty
           ; (q:qs) -> return (foldl (\ xs x -> Seq xs x) q qs)
           }
         }
    ; EOpt e b -> 
      {-
